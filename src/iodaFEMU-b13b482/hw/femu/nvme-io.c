...
{  //zhr-从NVME队列(sq)中取出请求
    NvmeSQueue *sq = opaque;
    FemuCtrl *n = sq->ctrl;

    uint16_t status;
    hwaddr addr;
    NvmeCmd cmd; //zhr-cmd相当于从NVME队列中取来的请求
    NvmeRequest *req;
    int processed = 0;

    nvme_update_sq_tail(sq);//zhr-更新提交队列尾指针
    while (!(nvme_sq_empty(sq))) {
        ...
        nvme_inc_sq_head(sq);//zhr-增加提交队列头指针

        req = QTAILQ_FIRST(&sq->req_list);//zhr-从提交队列中获取请求
        ...
        req->nvm_usrflag=((NvmeRwCmd *)&cmd)->rsvd2; //zhr-rsvd2是在内核中的一个修改，保留位用来接收nvm_usrflag
        if (n->print_log) {
            femu_debug("%s,cid:%d\n", __func__, cmd.cid);
        }

        status = nvme_io_cmd(n, &cmd, req);//zhr-处理请求
        if (1 && status == NVME_SUCCESS) {
            req->status = status;

            int rc = femu_ring_enqueue(n->to_ftl[index_poller], (void *)&req, 1);
            //zhr-将请求添加到环形队列中,在ftl.c的ftl_thread函数中处理
            if (rc != 1) {
    ...
    nvme_update_sq_eventidx(sq);//zhr-更新事件索引
    sq->completed += processed; //zhr-增加已完成的请求计数
}
...
//     // zhr-普通优先级队列的数组表示
//     NvmeRequest *normal_priority_queue[1024];
//     int npq_count = 0;
...
//         if (req->nvm_usrflag == 1) {
//          /*优先处理高优先级*/
//             status = nvme_io_cmd(n, &req->cmd, req); //zhr-处理请求
//             if (1 && status == NVME_SUCCESS) {
//                 req->status = status;
//                 int rc = femu_ring_enqueue(n->to_ftl[index_poller], (void *)&req, 1);
//                 if (rc != 1) {
...
//         } else {
//             // zhr-存储普通优先级请求
//             if (npq_count < MAX_QUEUE_SIZE) {
//                 normal_priority_queue[npq_count++] = req;
//             } else {
//                 femu_err("!\n");
...
//     }
//     // zhr-处理普通优先级队列中的请求
//     for (int i = 0; i < npq_count; i++) {
//         req = normal_priority_queue[i];
//         req->expire_time = req->stime = qemu_clock_get_ns(QEMU_CLOCK_REALTIME); // 重新赋值expire_time
//         status = nvme_io_cmd(n, &req->cmd, req); // zhr-处理请求
//         if (1 && status == NVME_SUCCESS) {
//             req->status = status;
//             int rc = femu_ring_enqueue(n->to_ftl[index_poller], (void *)&req, 1);
...
{   //zhr-将处理完成的请求插入到完成队列(cq),以便通知上层应用程序或驱动程序请求已经完成
    FemuCtrl *n = cq->ctrl;
    ...
    if (cq->phys_contig) {//zhr-物理连续
        addr = cq->dma_addr + cq->tail * n->cqe_size;
        ((NvmeCqe *)cq->dma_addr_hva)[cq->tail] = *cqe;
    } else {
        addr = nvme_discontig(cq->prp_list, cq->tail, n->page_size, n->cqe_size);
        nvme_addr_write(n, addr, (void *)cqe, sizeof(*cqe));//zhr-将一个完成队列条目（CQE）写入到某个地址空间中
    }

    nvme_inc_cq_tail(cq);//zhr- 增加完成队列尾指针
}

static void nvme_process_cq_cpl(void *arg, int index_poller)
{   //zhr-从 to_poller 队列中取出已经处理完成的请求，并插入完成队列 (CQ) 中
    FemuCtrl *n = (FemuCtrl *)arg; //zhr-FemuCtrl扮演NVME的角色
    NvmeCQueue *cq = NULL;
    NvmeRequest *req = NULL;
    struct rte_ring *rp = n->to_ftl[index_poller]; //zhr-指向环形缓冲区的指针
    pqueue_t *pq = n->pq[index_poller]; //zhr-指向优先级队列的指针
    uint64_t now;
    int processed = 0;
    int rc;

    if (BBSSD(n)) {//zhr-表示如果是特定设备(猜测是 Black Box SSD)
    ...
    while ((req = pqueue_peek(pq))) {
        ...
        n->should_isr[req->sq->sqid] = true;//zhr-标记需要ISR通知
    }
...
static void *nvme_poller(void *arg) //zhr-线程函数，用于轮询 NVMe 队列并处理 I/O 请求
{
    FemuCtrl *n = ((NvmePollerThreadArgument *)arg)->n;
    int index = ((NvmePollerThreadArgument *)arg)->index;

    switch (n->multipoller_enabled) {//zhr-根据 multipoller_enabled 标志的值，选择单一队列或遍历多个队列
    case 1:
        while (1) {
            ...
            if (sq && sq->is_active && cq && cq->is_active) {//zhr-只有活跃的队列才会被处理
...
void nvme_post_cqes_io(void *opaque)
{
    ...
    QTAILQ_FOREACH_SAFE(req, &cq->req_list, entry, next) { //zhr-遍历完成队列中的请求
        if (nvme_cq_full(cq)) {
            break;
        }
    ...
    if (processed > 0) {
        nvme_isr_notify_io(cq); //zhr-通知上层
    }
}
