...
typedef struct NvmeRequest {
    ...
    uint16_t                status;
    uint64_t                slba;//zhr-起始逻辑块地址，表示请求操作的起始位置
    uint16_t                is_write;
    uint16_t                nlb;//zhr-此请求操作的逻辑块数量
    uint16_t                ctrl;
    uint64_t                meta_size;//zhr-元数据
    uint64_t                mptr;
    void                    *meta_buf;
    uint64_t                oc12_slba;
    uint64_t                *oc12_ppa_list;
    NvmeCmd                 cmd;//zhr-命令结构体
    NvmeCqe                 cqe;
    uint8_t                 cmd_opcode;
    QEMUSGList              qsg;
    QEMUIOVector            iov;
    QTAILQ_ENTRY(NvmeRequest)entry;//zhr-请求队列条目
    int64_t                 stime;
    int64_t                 reqlat;
    int64_t                 gcrt;
    int64_t                 expire_time;

    uint64_t nvm_usrflag; /*zhr-nvme->usrflag*/

    /* OC2.0: sector offset relative to slba where reads become invalid */
    uint64_t predef;

    /* ZNS */
    void                    *opaque;//zhr-用户自定义的数据指针，可以存储任意数据
...
typedef struct NvmeSQueue {
    struct FemuCtrl *ctrl;//zhr-FemuCtrl与NvmeSQueue互相引用
    uint8_t     phys_contig;
    uint8_t     arb_burst;
    uint16_t    sqid;
    uint16_t    cqid;
...
typedef struct NvmeCQueue {
    ...
    uint8_t     phys_contig;//zhr-表示完成队列是否是物理连续的
    uint8_t     phase;
    uint16_t    cqid;//zhr-完成队列的id
    uint16_t    irq_enabled;//zhr-是否启用了中断
    uint32_t    head;
    uint32_t    tail;
    int32_t     virq;//zhr-中断请求编号
    uint32_t    vector;
    uint32_t    size;
    uint64_t    dma_addr;
    uint64_t    dma_addr_hva;
    uint64_t    *prp_list;//zhr-用于存储完成队列条目的物理页地址列表（如果队列不是物理连续的）
    EventNotifier guest_notifier;
    QEMUTimer   *timer;
    QTAILQ_HEAD(sq_list, NvmeSQueue) sq_list;//zhr-维护与此完成队列关联的提交队列（SQ）的链表
    QTAILQ_HEAD(cq_req_list, NvmeRequest) req_list;//zhr-维护完成队列中的请求条目的链表
    uint64_t    db_addr;
    uint64_t    db_addr_hva;
    uint64_t    eventidx_addr;
    uint64_t    eventidx_addr_hva;
...
typedef struct FemuCtrl {
    PCIDevice       parent_obj; //zhr-用于处理与 PCI 总线相关的操作
    MemoryRegion    iomem;
    ...
    QemuThread  *poller;//zhr-一个指向 QEMU 线程对象的指针
    bool        dataplane_started;//zhr-数据平面
    bool        vector_poll_started;//zhr-向量轮询
    ...
    NvmeSQueue      **sq;
    NvmeCQueue      **cq;//zhr-sq和cq指向 NVMe 队列的指针数组，用于管理设备的提交队列和完成队列
    NvmeSQueue      admin_sq;
    NvmeCQueue      admin_cq;
    ...
    pqueue_t        **pq;
    bool            *should_isr;//zhr-指示是否需要中断服务的标志数组，用于通知请求的完成情况。
    bool            poller_on;//zhr-用于标记轮询器的状态，表示是否开启轮询器。
