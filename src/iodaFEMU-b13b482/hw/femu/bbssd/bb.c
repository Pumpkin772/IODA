...
static void bb_flip(FemuCtrl *n, NvmeCmd *cmd)
{
    ...
    case FEMU_DISABLE_LOG:
        ...
        break;
    case FEMU_WAP_STATIS:
    //femu_log("write%d,此次io时ssd是否GC%d,已使用页数%d,GC写入量%d,用户写入量%d\n",ssd->write_cnt,ssd->flag,ssd->total_cnt ,ssd->gc_cnt,ssd->io_cnt);
    //femu_log("GC_flag%d,used_page%d,GC_page%d,io_page%d\n",ssd->flag,ssd->total_cnt ,ssd->gc_cnt,ssd->io_cnt);
    femu_log("used_pages%d,total_pages%d,GC_pages_w%d,gc_blks_e%d,write%d\n",ssd->used_cnt,ssd->total_cnt ,ssd->gc_pages_write,ssd->gc_blks_erase,ssd->write_cnt);
    break;
    default:
